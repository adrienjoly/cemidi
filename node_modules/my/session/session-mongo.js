...
function SessionMongo(options) {
  ...
  var self = this;  
  var db = 'mongodb://' 
    + (options.user && options.password ? options.user + ':' + options.password + '@' : '')
    + (options.host || 'localhost') + ':' + (options.port || 27017) + '/'
  ...
  if (options.cleanEvery === 'day')
    options.cleanEvery = 24 * 60 * 60 * 1000;
  if (options.cleanEvery === 'hour')
    options.cleanEvery = 60 * 60 * 1000;
  
  if (typeof options.cleanEvery === 'number') {
    setTimeout(function() {
      if (self.collection)
        self.collection.remove({'cookie.expires': {$lte: Date.now()}});
    }, options.cleanEvery);
  }
...
SessionMongo.prototype.get = function(id, cb) {
  ...
  this.collection ? 
    ...
    this.pendingRequests.push({action: 'findOne', args: [{_id: id}, callback]});  
}
