  ...
  var fakeRequest = function () {
    ...
    for(var i in arguments)
      if (typeof arguments[i] == "string" && arguments[i].indexOf("http") == 0)
        url = arguments[i]./*replace("alt=jsonc", "alt=json").*/replace(/[&?]callback=\?/, "");
      else if (typeof arguments[i] == "function")
        callback = arguments[i];

    if (url || url.indexOf("https") == 0) {
      console.log("transforming https to http");
...
Page.prototype.getImages = function() {
  ...
  for (i = 0, len = imgs.length; i < len; i++) {
    ...
    if (img.substr(0, 7) !== 'http://') {
      if (base)
        img = path.normalize(base + '/' + img);
      else
        img = 'http://' + path.normalize(img.charAt(0) === '/' ?
          (this.host + '/' + img) :
...
Page.prototype.getMp3s = function() {
  ...
  for (i = 0, len = mp3s.length; i < len; i++) {
    ...
    if (mp3.substr(0, 7) !== 'http://') {
      if (base)
        mp3 = path.normalize(base + '/' + mp3);
      else
        mp3 = 'http://' + path.normalize(mp3.charAt(0) === '/' ?
          (this.host + '/' + mp3) :
...
Page.prototype.extractEmbeds = function(cb) {
  ...
  var embedRefs = [], remaining = embedUrls.length;
  if (!remaining)
    return cb(embedRefs);
  ...
  for (var i in embedUrls)
    embedDetector.extractEmbedRef(embedUrls[i], function(embedRef) {
      ...
      if (embedRef && embedRef.id)
        embedRefs.push({
          id: embedRef.id,
          name: embedRef.name && embedRef.name != "undefined" ? embedRef.name : null,
          url: embedRef.url
        });
      if (!--remaining)
        return cb(embedRefs);
  ...
  return http.get(options, function(res) {
    ...
    if (location) {
      ...
      res.on('end', function() {
        ...
        if (charset !== 'utf-8')
          try {
            text = new iconv.Iconv(charset, 'utf-8').convert(buffer).toString('utf8');
          } catch (e) {}
...
getPage.ContentType = function(address, callback) {
  ...
  httpOrHttps.request(options, function(res) {
    ...
    if (location) {
      options.path = location;
      httpOrHttps.request(options, arguments.callee).on('error', callback).end();
    } else {
      contentType =
        headers['content-type'] ||
        headers['Content-Type'] ||
        headers['Content-type'];
      callback(null, contentType && CONTENT_TYPE_REG.test(contentType) ?
        RegExp.$1 :
        'noContentType'
      );
    }
